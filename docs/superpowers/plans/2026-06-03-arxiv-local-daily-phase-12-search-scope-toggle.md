...
- 2026-06-03: The UI should make the scope explicit with a `当日 / 总览` switch.
- 2026-06-03: `当日` keeps sending the selected date to `/api/search/papers`; `总览` omits `date` and searches the whole local database.

## Tasks

- [x] Add web tests for the scope control and date/overview search behavior.
- [x] Add a `search-scope` select with `当日` as the default and `总览` as the overview option.
